(function(root){
	...
	var makeMemoryWatcher = function(dcpu, change_mem_peeker){
		...
		mem_poker.size = 5;
		var goto = div.appendChild(document.createElement('button'));
		goto.innerHTML = 'jmp';
		goto.style.fontSize = '50%';
		...
		for (var i = 0; i < 16; i++){
			...
			mem_matrix_labels[i].style.backgroundColor = '#DDD';
			for (var j = 0; j < 8; j++){
		...
		setInterval(function(){
			...
			for (var i = 0; i < 8 * 16; i++){
				...
				if (address == dcpu.mem.pc)
					n = "<span style='border: 1px solid red'>" + n + "</span>";
				if (address == dcpu.mem.sp)
					n = "<span style='border: 1px solid blue'>" + n + "</span>";
